...
Sơ đồ sau minh họa ứng dụng tiêu biểu của Supplier là khởi tạo lười biếng: giá trị chỉ được tạo khi `get()` được gọi và chưa có sẵn trong cache.

```mermaid
flowchart TD
    A["Gọi get()"] --> B{"Đã có giá trị cache?"}
    B -->|Có| C["Trả về giá trị sẵn"]
