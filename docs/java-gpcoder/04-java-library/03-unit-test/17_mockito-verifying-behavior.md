...
- ⭐ **`verify(mock)` kiểm tra hành vi** — phương thức có được gọi không, gọi mấy lần, với tham số gì.
- **Verification mode** — `times(n)`, `never()`, `atLeastOnce()`, `atLeast`/`atMost`, `only()`.
- **Đảm bảo không gọi thừa** — `verifyNoInteractions` / `verifyNoMoreInteractions`.
- **`ArgumentCaptor` bắt tham số truyền vào** — để assert chi tiết; `InOrder` kiểm tra thứ tự gọi.
- **`timeout(...)`** — xác nhận trong giới hạn thời gian cho tác vụ bất đồng bộ.
