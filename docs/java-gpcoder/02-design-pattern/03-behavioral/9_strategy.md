...
- ⭐ **`Strategy` đóng gói mỗi thuật toán vào một lớp riêng và cho phép hoán đổi tại runtime** — thuật toán thay đổi độc lập với client dùng nó.
- ⭐ **`Context` ủy quyền cho interface `Strategy`** — `Sorter` giữ tham chiếu `SortStrategy` và đổi thuật toán (`BubbleSort`, `SelectionSort`, `BuiltInSort`) qua `setStrategy()` mà không sửa code.
- **Loại bỏ `if-else` chọn thuật toán** — tuân thủ Open/Closed, dễ kiểm thử riêng từng chiến lược.
- **Nhược điểm** — client cần biết khác biệt giữa các chiến lược để chọn đúng, tăng số lượng lớp.
- **Khi nào dùng** — phương thức thanh toán, thuật toán nén, định tuyến, validator, bộ lọc.
