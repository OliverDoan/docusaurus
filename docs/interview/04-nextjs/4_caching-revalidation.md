...
2. **Runtime**: trang được serve static; sau khoảng `revalidate` giây, request tiếp theo trigger **regenerate ở background** theo kiểu **stale-while-revalidate**: request đầu tiên sau khi hết hạn vẫn nhận bản cũ ngay lập tức, Next âm thầm fetch lại, request sau mới nhận bản mới — không user nào phải chờ origin.
3. **On-demand**: `revalidatePath`/`revalidateTag` regenerate ngay khi giá đổi (xem Câu 24).
...
## Câu 24: On-demand revalidation trong Next.js là gì? `[Intermediate]`

### Câu hỏi

> Time-based revalidation phải chờ hết hạn mới làm mới data. Nếu muốn data cập nhật ngay lập tức sau khi user sửa nội dung hoặc CMS publish bài mới thì dùng cơ chế gì? On-demand revalidation hoạt động ra sao và gọi ở đâu?

### Giải thích lý thuyết

On-demand revalidation = **chủ động purge Data Cache ngay tại thời điểm biết data đã đổi**, thay vì chờ hết hạn như time-based.

**Khác time-based thế nào:**

| Tiêu chí          | Time-based (`next.revalidate`)                       | On-demand (`revalidatePath`/`revalidateTag`)      |
| ----------------- | ---------------------------------------------------- | ------------------------------------------------- |
| Thời điểm làm mới | Theo nhịp định trước (60s, 1h...)                    | Ngay khi gọi — biết chính xác lúc data đổi        |
| Hành vi           | Stale-while-revalidate: request đầu sau hết hạn vẫn nhận **data cũ**, regenerate ở background | **Purge ngay** — request tiếp theo chắc chắn fetch mới (user đó chờ origin) |
| Phù hợp           | Data đổi theo nhịp dự đoán được: tin tức, bảng giá   | Sau mutation, webhook từ CMS                      |

**Hai API on-demand:**
- `revalidatePath('/posts')` — purge cache theo **đường dẫn**: mọi data của route đó. Chỉ cần biết URL, không cần chuẩn bị trước.
- `revalidateTag('posts')` — purge theo **tag**: mọi fetch (ở bất kỳ route nào) đã gắn `next: { tags: ['posts'] }` từ trước. Invalidate theo chiều data thay vì chiều trang. (So sánh sâu hai API này ở Câu 48.)

**Gọi ở đâu — chỉ trên server, 2 nơi:**
1. **Server Action** — sau mutation: ngoài purge server cache, Next còn **làm mới Router Cache của client đang thực hiện action** → UI cập nhật ngay trong cùng round-trip, không cần `router.refresh()`.
2. **Route Handler** — nhận webhook từ CMS/hệ thống ngoài: chỉ purge server cache; client đang mở trang thấy data mới ở lần navigation/refresh sau. Nhớ verify secret của webhook.

Không gọi được trong lúc render component.

Pitfall: `revalidateTag` chỉ tác dụng với fetch đã gắn tag (hoặc query wrap `unstable_cache` kèm tags) — DB query trần qua Prisma thì vô tác dụng.

Production thường **combine**: time-based làm safety net + on-demand để update tức thì sau mutation.

### Code minh hoạ

```typescript
// Điều kiện tiên quyết cho revalidateTag: gắn tag lúc fetch
...
// 1. On-demand trong Server Action — sau mutation
// app/actions/post.ts
"use server";
...
export async function publishPost(formData: FormData) {
  ...
  // Bonus: Router Cache của client gọi action cũng được làm mới ngay
}

// 2. On-demand qua Route Handler — webhook từ CMS
...
//   t=90s : request → trả data CŨ ngay + background fetch → request sau mới fresh
// On-demand:
//   gọi revalidateTag → purge NGAY → request tiếp theo fetch origin (chờ) → fresh
```

### Đáp án mẫu

> "On-demand revalidation là chủ động purge Data Cache **ngay tại thời điểm biết data đổi**, qua hai API `revalidatePath` — purge theo đường dẫn — và `revalidateTag` — purge mọi fetch đã gắn tag từ trước. Khác biệt với time-based: time-based chạy theo nhịp và có hành vi stale-while-revalidate — request đầu sau hết hạn vẫn nhận data cũ, regenerate ở background; còn on-demand purge ngay, request tiếp theo chắc chắn fetch mới. Hai API này chỉ gọi được trên server, ở hai chỗ: **Server Action** sau mutation — điểm cộng là Router Cache phía client cũng được làm mới trong cùng round-trip, UI cập nhật ngay; và **Route Handler** nhận webhook từ CMS — nhớ verify secret. Production em combine cả hai: time-based làm safety net, on-demand để update tức thì sau mutation."

---

## Câu 25: fetch cache trong Next.js App Router hoạt động như thế nào? `[Intermediate]`
...
**force-cache vs no-store — không chỉ là chuyện cache:**
- `force-cache`: response lưu Data Cache, share mọi user → route có thể **prerender static**, serve từ CDN. Phù hợp data ít đổi: categories, config, blog post.
- `no-store`: bỏ qua cache, gọi origin mỗi request. Quan trọng: **1 fetch `no-store` làm cả route segment thành dynamic** — render lại mỗi request, mất Full Route Cache. Một dòng code đổi behavior cả trang; data realtime nên cô lập vào component riêng wrap `Suspense` để shell vẫn static.
...
// no-store: 1 fetch làm CẢ route thành dynamic → cô lập bằng Suspense
...
Cách hình dung: `revalidatePath` trả lời "**trang nào** cần mới?" — invalidate theo chiều route; `revalidateTag` trả lời "**data nào** đã đổi?" — Next tự tìm mọi nơi dùng data đó.
...
1. **Thiết kế tag hierarchy theo entity:**
   ...
   - Đây là điểm thắng quyết định: thêm trang mới dùng data này chỉ cần gắn đúng tag, **không phải sửa code mutation**. Dùng path thì phải nhớ liệt kê đủ mọi trang — dễ sót khi codebase lớn lên.
...
2. **`revalidatePath` và 2 mode `'page'` / `'layout'`:**
   ...
   - Path hợp lý khi data chỉ hiện đúng 1 trang (ví dụ trang About) — đơn giản, không cần bày tag.
...
> "Khác nhau ở **đơn vị invalidate**: `revalidatePath` purge theo đường dẫn — chỉ cần biết URL nhưng phải tự liệt kê đủ mọi trang bị ảnh hưởng; `revalidateTag` purge theo tag gắn vào fetch từ trước — em nói 'data này đã đổi' và Next tự làm mới mọi nơi dùng nó. Chiến lược của em là **tag-first theo entity**: mỗi sản phẩm có tag riêng `product-{id}` và tag collection `products`; mutation chỉ cần `revalidateTag` 2 dòng là detail, list, sidebar, admin đều fresh — thêm trang mới chỉ gắn tag lúc fetch, không sửa code mutation. Vì sidebar nằm trong layout và data lấy từ Prisma chứ không phải fetch, em wrap query bằng `unstable_cache` kèm tags — pitfall nhiều người dính. `revalidatePath` em để cho trang đơn lẻ như About, dùng đúng dạng: URL literal, `('/products/[slug]', 'page')`, hay `'layout'` mode — tuyệt đối tránh `revalidatePath('/', 'layout')` vì purge cả site. Em ưu tiên gọi trong Server Action vì nó làm mới luôn Router Cache phía client."
...
2. **Data Cache** — của Next: lưu response fetch trên server, share mọi user, sống qua request và qua deploy. Đây là lớp điều khiển bằng `cache`, `next.revalidate`, `next.tags` (Câu 24, 25). Next 15: **opt-in**.
...
## Câu 66: Parallel và sequential data fetching khác nhau ra sao trong App Router, và preload pattern giải quyết vấn đề gì? `[Intermediate]`

### Câu hỏi

> Trang profile cần 3 nguồn data: thông tin user, danh sách bài viết, và thống kê. Em viết 3 dòng `await` liên tiếp thì trang chậm. Giải thích vấn đề waterfall, cách fetch song song, preload pattern giải quyết vấn đề gì, và khi nào sequential là bắt buộc?
...
- **Preload pattern**: giải quyết đúng bài toán **waterfall ngầm cha–con**. Export hàm `preload()` gọi sớm ở cha để "hâm nóng" request memoization (fetch memoization hoặc React `cache()`), con render sau gọi lại cùng function sẽ **hit memo ngay** — phá waterfall mà không phải truyền props/promise xuyên cây component. Vì memo theo render pass nên không lo stale data.
...
> "Sequential là các request chạy nối đuôi — 3 dòng `await` liên tiếp với request 300ms thì trang mất 900ms; parallel là khởi động cùng lúc, tổng thời gian bằng request chậm nhất. Fix cơ bản là `Promise.all`, nhưng em hay dùng pattern linh hoạt hơn: **khởi tạo promise sớm rồi await sau** — gọi function không await để request bắt đầu chạy, await ở đúng chỗ cần kết quả, hoặc truyền thẳng promise xuống Client Component dùng `use()` kết hợp Suspense để stream. Cái khó thấy hơn là **waterfall ngầm cha–con**: con chỉ fetch sau khi cha render xong — đây chính là bài toán preload pattern giải quyết: export hàm `preload()` gọi ở cha để hâm nóng `cache()` memo, con gọi lại là hit memo ngay, không cần truyền props. Sequential chỉ bắt buộc khi request sau **phụ thuộc kết quả** request trước, ví dụ cần `user.teamId` mới fetch được team — lúc đó em gộp ở backend thành 1 query hoặc bọc Suspense để phần đó không chặn cả trang."
...
| "On-demand revalidation gọi được ở mọi nơi"                   | Chỉ trong **Server Action** hoặc **Route Handler**, không gọi lúc render          |
| "revalidateTag hoạt động với mọi DB query"                    | Chỉ với fetch có tags hoặc query wrap `unstable_cache` kèm tags                   |
| "ISR tự refresh trang theo cron ngầm"                         | Time-based revalidation chỉ chạy khi **có traffic** — không ai vào thì giữ bản cũ |
